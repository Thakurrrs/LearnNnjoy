...
function MountainRescue({ onFinish }: { onFinish: () => void }) {
  ...
  return <>
    ...
    {step === 0 && <section className="chapter-event"><p className="activity-prompt">A storm has knocked Nova&apos;s rescue pod from the <b>+3</b> cliff marker. It slides <b>7 levels down</b> into the fog. Your map uses numbers above and below sea level.</p><div className="market-scene"><span>🏔️</span><span>🌧️</span><span>🚁</span><b>+3</b></div><button className="primary" onClick={() => setStep(1)}>Open the cliff map →</button></section>}
    {step === 1 && <section className="chapter-event"><p className="activity-prompt">Move Nova down seven equal levels. Cross zero if the trail takes you there.</p><div className="number-line-lab" aria-label={`Nova is currently at ${position}`}>{Array.from({ length: 17 }, (_, index) => index - 8).map((value) => <button key={value} onClick={() => setPosition(value)} className={value === position ? "active" : ""} aria-label={`Move Nova to ${value}`}><i>{value === position ? "✦" : ""}</i><span>{value > 0 ? `+${value}` : value}</span></button>)}</div><div className="activity-controls"><button onClick={() => setPosition((value) => Math.max(-8, value - 1))}>← Go down one</button><b>{position > 0 ? `+${position}` : position}</b><button onClick={() => setPosition((value) => Math.min(8, value + 1))}>Go up one →</button></div>{complete && <div className="mini-discovery"><b>The pod lands at −4.</b><span>Going down from +3 by 7 steps crosses zero.</span></div>}<button className="primary" disabled={!complete} onClick={() => setStep(2)}>Read the rescue marker →</button></section>}
...
function BalanceLab({ onFinish }: { onFinish: () => void }) {
  ...
  return <>
    ...
    {step === 0 && <section className="chapter-event"><p className="activity-prompt">Nova&apos;s supply crate will open only when its energy scale is balanced. A mystery crate plus <b>5 blocks</b> matches <b>12 blocks</b>.</p><div className="market-scene"><span>📦</span><span>⚖️</span><span>✦</span><b>? + 5 = 12</b></div><button className="primary" onClick={() => setStep(1)}>Enter the balance lab →</button></section>}
    {step === 1 && <section className="chapter-event"><p className="activity-prompt">Remove the <b>same</b> number of blocks from both sides until the mystery crate stands alone.</p><div className="balance-lab" aria-label="A balanced equation"><div><b>?</b>{Array.from({ length: 5 - removed }, (_, i) => <i key={i}>✦</i>)}</div><strong>⚖️</strong><div>{Array.from({ length: 12 - removed }, (_, i) => <i key={i}>✦</i>)}</div></div><p className="equation-readout">? + {5 - removed} = {12 - removed}</p><div className="activity-controls"><button disabled={removed === 0} onClick={() => setRemoved((value) => value - 1)}>Put one back</button><button className="primary" disabled={complete} onClick={() => setRemoved((value) => value + 1)}>Remove one from both sides</button></div>{complete && <div className="mini-discovery"><b>The crate is alone: ? = 7.</b><span>Both sides changed equally, so the balance stayed true.</span></div>}<button className="primary" disabled={!complete} onClick={() => setStep(2)}>Check the balance rule →</button></section>}
...
function Skatepark({ onFinish }: { onFinish: () => void }) {
  ...
  return <>
    ...
    {step === 0 && <section className="chapter-event"><p className="activity-prompt">Nova is designing a rooftop skatepark for the neighbourhood. The first ramp must join the triangle course with a safe <b>60° turn</b>.</p><div className="market-scene"><span>🛹</span><span>🏙️</span><span>🔺</span><b>60°</b></div><button className="primary" onClick={() => setStep(1)}>Inspect the ramp plan →</button></section>}
    {step === 1 && <section className="chapter-event"><p className="activity-prompt">Turn the ramp until it makes a <b>60°</b> angle with the ground. The course triangle will then lock into place.</p><div className="skate-lab"><div className="ground" /><div className="ramp" style={{ transform: `rotate(${-angle}deg)` }} /><div className={complete ? "triangle-frame complete" : "triangle-frame"}>△</div><b>{angle}°</b></div><input className="discount-slider" aria-label="Ramp angle" type="range" min="0" max="120" step="10" value={angle} onChange={(event) => setAngle(Number(event.target.value))} /><div className="activity-controls"><button onClick={() => setAngle((value) => Math.max(0, value - 10))}>Rotate back</button><b>{angle}°</b><button onClick={() => setAngle((value) => Math.min(120, value + 10))}>Rotate forward</button></div>{complete && <div className="mini-discovery"><b>The ramp turns 60° from the ground.</b><span>An angle measures a turn, not a distance.</span></div>}<button className="primary" disabled={!complete} onClick={() => setStep(2)}>Complete the triangle →</button></section>}
...
function CricketData({ onFinish }: { onFinish: () => void }) {
  ...
  return <>
    ...
    {step === 0 && <section className="chapter-event"><p className="activity-prompt">The neighbourhood cricket final starts at sunset. Nova must choose a three-player practice squad using the match-score chart—not a hunch.</p><div className="market-scene"><span>🏏</span><span>📊</span><span>🌆</span><b>3 places</b></div><button className="primary" onClick={() => setStep(1)}>Open the score board →</button></section>}
