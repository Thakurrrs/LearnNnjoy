...
export default function Home() {
  ...
  const pausedAtChapterMap = (
    pausedAdventure?.interactionState.kind === "skatepark"
    || pausedAdventure?.interactionState.kind === "mountain"
    || pausedAdventure?.interactionState.kind === "moonbase"
    || pausedAdventure?.interactionState.kind === "balance"
  ) && pausedAdventure.interactionState.chapterMapOpen;
  ...
  const ageFraming = grade <= 6
    ...
    : grade <= 9
      ...
      : { role: "Navigator", object: "star compass", world: "Lumina navigation brief" };
  ...
  function openSkateparkQuestMap() {
    setActiveAdventure("skatepark");
    setSelectedChapter("skatepark");
    setActivityMode("live");
    setGradeSevenProgress((progress) => {
      const opened = openSavedGradeSevenAdventure(progress, "skatepark");
      const record = opened.skatepark;
      if (!record || record.interactionState.kind !== "skatepark") return opened;
      return updateGradeSevenAdventure(opened, "skatepark", {
  ...
  function openMountainQuestMap() {
    setActiveAdventure("mountain");
    setSelectedChapter("mountain");
    setActivityMode("live");
    setGradeSevenProgress((progress) => {
      const opened = openSavedGradeSevenAdventure(progress, "mountain");
      const record = opened.mountain;
      if (!record || record.interactionState.kind !== "mountain") return opened;
      return updateGradeSevenAdventure(opened, "mountain", {
        ...record.interactionState,
        chapterMapOpen: true,
      }, record.completed);
    });
  ...
  function openMoonbaseQuestMap() {
    setActiveAdventure("moonbase");
    setSelectedChapter("moonbase");
    setActivityMode("live");
    setGradeSevenProgress((progress) => {
      const opened = openSavedGradeSevenAdventure(progress, "moonbase");
      const record = opened.moonbase;
      if (!record || record.interactionState.kind !== "moonbase") return opened;
      return updateGradeSevenAdventure(opened, "moonbase", {
        ...record.interactionState,
        chapterMapOpen: true,
        activeQuest: null,
      }, record.completed);
  ...
  function openBalanceQuestMap() {
    setActiveAdventure("balance");
    setSelectedChapter("balance");
    setActivityMode("live");
    setGradeSevenProgress((progress) => {
      const opened = openSavedGradeSevenAdventure(progress, "balance");
      const record = opened.balance;
      if (!record || record.interactionState.kind !== "balance") return opened;
      return updateGradeSevenAdventure(opened, "balance", {
        ...record.interactionState,
        chapterMapOpen: true,
      }, record.completed);
    });
  ...
  function previousGradeSevenScene() {
    if (activityMode !== "live" || !canGoToPreviousGradeSevenEvent(activeActivityState)) return;
  ...
  function openCurrentAdventureMap() {
    if (
      activityMode === "live"
      && (
        activeActivityState.kind === "skatepark"
        || activeActivityState.kind === "mountain"
        || activeActivityState.kind === "moonbase"
        || activeActivityState.kind === "balance"
      )
      && !activeActivityState.chapterMapOpen
    ) {
      changeGradeSevenActivity({ ...activeActivityState, chapterMapOpen: true });
      return;
    }
    setScreen("adventures");
  }

  function finishGradeSevenAdventure(finalState: GradeSevenInteractionState = activeActivityState) {
    if (activityMode === "replay") {
      setScreen("journal");
      return;
    }
    ...
    setGradeSevenProgress((progress) => updateGradeSevenAdventure(progress, activeAdventure, finalState, true));
    ...
    return <main className={`shell welcome-shell ${gradeTheme}`}>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <h1>This world needs a hero. That&apos;s you.</h1>
          <p className="lede">Real adventures with Nova, your star friend — who needs your help with the maths that saves the day. Grades 4–12.</p>
  ...
  if (screen === "story") {
    ...
    return <main className={`story-shell ${gradeTheme} beat-${storyBeat}`}><Image src="/images/lumina-bridge.png" alt="Nova waits by a broken starlight bridge in the world of Lumina." fill priority sizes="100vw" className="story-art" /><div className="story-vignette" /><nav className="story-nav"><div className="brand"><span>✦</span> LearnNnjoy</div><span>{ageFraming.world} · {storyBeat + 1}/4</span><button className="text-button home-button" onClick={goHome}>⌂ Home</button></nav><section className="story-player">
      {storyBeat === 0 && <div className="story-dialogue"><p className="eyebrow">CHAPTER ONE · YOUR ADVENTURE</p><h1>Nova zooms down to you.</h1><p>{`"${name.trim() || "Hey"}! I found ONE ${ageFraming.object}. Mira and I both want it. Help us share it fairly?"`}</p><button className="primary" onClick={() => setStoryBeat(1)}>I&apos;ll help you, Nova →</button></div>}
      {storyBeat === 1 && <div className="story-dialogue"><p className="eyebrow">MAKE A FAIR SHARE</p><h1>{!fruitSplit ? "Here's the moon-fruit." : !fruitShared ? "Two equal pieces! Pick mine." : "Nova hugs the piece. \"One-half!\""}</h1><p>{!fruitSplit ? "Tap it to make one fair cut through the middle." : !fruitShared ? "Both pieces match. Tap one to give it to Nova." : "One whole, split into 2 equal parts. Nova has 1 of 2: one-half, written 1/2."}</p><div className={`${fruitSplit ? "moon-fruit split" : "moon-fruit"} ${gradeTheme}`} aria-label={`One whole ${ageFraming.object} that can be split into two equal parts`}>{!fruitSplit ? <button type="button" aria-label={`Split the whole ${ageFraming.object} fairly`} onClick={() => setFruitSplit(true)}>✦</button> : <><button type="button" className={fruitShared ? "shared" : ""} aria-label="Give this equal half to Nova" onClick={() => setFruitShared(true)}>✦</button><button type="button" className={fruitShared ? "remaining" : ""} aria-label="The other equal half">✦</button></>}</div><p className="story-helper">{!fruitSplit ? "One whole · tap to split it" : !fruitShared ? "2 equal pieces · pick one for Nova" : "1 of 2 equal parts = 1/2"}</p>{fruitShared && <button className="primary" onClick={() => setStoryBeat(2)}>To the bridge! →</button>}</div>}
      {storyBeat === 2 && <div className="story-dialogue"><p className="eyebrow">TRY THE IDEA AGAIN</p><h1>The bridge wants half its panels lit.</h1><p>Nova whispers: &quot;One-half can look different here. Light 2 of the 4 equal panels!&quot;</p><div className="bridge-cells" aria-label="Four equal bridge energy chambers">{[0, 1, 2, 3].map((cell) => <button type="button" key={cell} className={storyCells.includes(cell) ? "charged" : ""} onClick={() => toggleStoryCell(cell)} aria-pressed={storyCells.includes(cell)}><span>✦</span></button>)}</div><p className="story-helper">{bridgeReady ? "2 of 4 lit — that's one-half too!" : `${storyCells.length} of 4 panels lit`}</p><button className="primary" disabled={!bridgeReady} onClick={() => setStoryBeat(3)}>Send the starlight →</button></div>}
      {storyBeat === 3 && <div className="story-dialogue resolved"><p className="eyebrow">THE BRIDGE WAKES UP</p><h1>You did it — Nova is dancing.</h1><p>{`"See, ${name.trim() || "friend"}? 1 of 2, or 2 of 4 — BOTH are one-half. You're good at this!"`}</p><div className="story-reveal">✦ ✦ <span>✦ ✦</span></div><button className="primary" onClick={() => setScreen("diagnostic")}>Keep going with Nova →</button></div>}
    </section></main>;
  }
  ...
  if (screen === "adventures") {
    ...
    const selectedAtQuestMap = (
      selectedProgress?.interactionState.kind === "skatepark"
      || selectedProgress?.interactionState.kind === "mountain"
      || selectedProgress?.interactionState.kind === "moonbase"
      || selectedProgress?.interactionState.kind === "balance"
    ) && selectedProgress.interactionState.chapterMapOpen;
  ...
  if (screen === "activity") {
    const onSkateparkQuestMap = activeActivityState.kind === "skatepark" && activeActivityState.chapterMapOpen;
    const onMountainQuestMap = activeActivityState.kind === "mountain" && activeActivityState.chapterMapOpen;
    const onMoonbaseQuestMap = activeActivityState.kind === "moonbase" && activeActivityState.chapterMapOpen;
    const onBalanceQuestMap = activeActivityState.kind === "balance" && activeActivityState.chapterMapOpen;
    return <main className={`shell activity-shell theme-pathfinder world-theme-${activeAdventure}`}><nav className="topbar"><div className="brand"><span>✦</span> LearnNnjoy</div><div className="quest-stats"><HeroDuo avatar={avatar} equippedCosmetic={equippedCosmetic} level={explorer.level} pet={pet} size="sm" /><button className="text-button" aria-label={muted ? "Turn sounds on" : "Turn sounds off"} onClick={() => setMuted(sound.toggleMuted())}>{muted ? "🔇" : "🔊"}</button>{activityMode === "live" && canGoToPreviousGradeSevenEvent(activeActivityState) && <button className="text-button previous-scene" onClick={previousGradeSevenScene}>← Previous scene</button>}<button className="text-button home-button" onClick={goHome}>⌂ Home</button><button className="text-button" onClick={() => activityMode === "replay" ? openGradeSevenJournal(activeAdventure) : openCurrentAdventureMap()}>{activityMode === "replay" ? "Story journal" : onSkateparkQuestMap || onMountainQuestMap || onMoonbaseQuestMap || onBalanceQuestMap ? "Maths star map" : activeAdventure === "skatepark" ? "Skatepark quests" : activeAdventure === "mountain" ? "Mountain quests" : activeAdventure === "moonbase" ? "Moonbase quests" : activeAdventure === "balance" ? "Balance quests" : "Adventure map"}</button></div></nav><GradeSevenActivity id={activeAdventure} state={activeActivityState} mode={activityMode} firstTime={shouldAwardGradeSevenCompletion(activityMode, completedAdventures.includes(activeAdventure))} heroName={name} avatar={avatar} equippedCosmetic={equippedCosmetic} onChange={changeGradeSevenActivity} onFinish={finishGradeSevenAdventure} /></main>;
  }
  ...
  if (screen === "map") {
    return <main className={`shell dashboard-shell ${gradeTheme}`}><nav className="topbar"><div className="brand"><span>✦</span> LearnNnjoy</div><div className="nav-actions"><button className="text-button" onClick={openGradePicker}>Switch grade</button><button className="text-button" onClick={() => setScreen(grade === 7 ? "adventures" : "quest")}>Continue adventure</button></div></nav>
      ...
      {pausedAdventure && pausedAdventureCopy && <section className="resume-adventure-card"><span>{pausedAdventureCopy.icon}</span><div><p className="eyebrow">YOUR STORY IS SAFE</p><h2>Resume {pausedAdventureCopy.title}</h2><p>{pausedAtChapterMap ? "Your connected chapter quests are waiting exactly where you left them." : `Event ${pausedAdventure.lastEvent + 1} is waiting exactly where you left it.`}</p></div><button className="primary" onClick={() => openGradeSevenAdventure(activeAdventure)}>{pausedAtChapterMap ? activeAdventure === "mountain" ? "Open Mountain quests →" : activeAdventure === "moonbase" ? "Open Moonbase quests →" : activeAdventure === "balance" ? "Open Balance quests →" : "Open Skatepark quests →" : `Resume event ${pausedAdventure.lastEvent + 1} →`}</button></section>}
  ...
  if (screen === "path") {
    ...
    return <main className={`shell completion-shell ${gradeTheme}`}><nav className="topbar"><div className="brand"><span>✦</span> LearnNnjoy</div><div className="nav-actions"><div className="pill">Nova found your trail</div><button className="text-button home-button" onClick={goHome}>⌂ Home</button></div></nav><section className="trail-card"><div className={`trail-emblem ${trail.id}`}>{trail.id === "visual" ? "◐" : trail.id === "guided" ? "✦" : "✧"}</div><p className="eyebrow">YOUR STARTING TRAIL</p><h1>{trail.label}</h1><p>{trail.message}</p><div className="trail-support"><b>How Nova will help</b><span>{trail.support}</span></div><button className="primary" onClick={() => { setShowHint(trail.id === "visual"); setScreen("chapter"); }}>Begin my Lumina mission →</button></section></main>;
  }

  if (screen === "chapter") {
    const sceneImage = current.visual === "number-line" ? "/images/lumina-mist-trail.png" : "/images/lumina-bridge.png";
    return <main className={`chapter-shell chapter-${current.visual} ${gradeTheme}`}><Image src={sceneImage} alt="A chapter of Nova's learning adventure begins." fill priority sizes="100vw" className="chapter-art" /><div className="chapter-overlay" /><nav className="story-nav"><div className="brand"><span>✦</span> LearnNnjoy</div><span>{subjectMissionName} · discovery {questIndex + 1} of {gradeQuests.length}</span><button className="text-button home-button" onClick={goHome}>⌂ Home</button></nav><section className="chapter-dialogue"><p className="eyebrow">CHAPTER {activeMissionChapter.number} · {activeMissionChapter.topic}</p><h1>{lessonStory.chapterTitle}</h1><p>{personalize(lessonStory.chapterDialogue, name)}</p><StoryReel story={lessonStory} />{current.visual === "ratio" && <div className="chapter-groups" aria-label="One matching group becomes two matching groups"><div>✦✦<small>one group</small></div><strong>→</strong><div>✦✦✦✦<small>two matching groups</small></div></div>}{current.visual === "fraction" && <div className="chapter-whole" aria-label="One whole divided into two equal pieces"><span /><span /></div>}<div className="chapter-progress"><span style={{ width: `${((questIndex + 1) / gradeQuests.length) * 100}%` }} /></div><button className="primary" onClick={() => setScreen("quest")}>{lessonStory.chapterAction} →</button></section></main>;
  }
